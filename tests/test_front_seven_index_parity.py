"""
Parity test for prediction_audit/engine/front_seven_index.py against v35's own real
recalculated Front Seven Index values -- 32 real teams, 3 real metrics.
"""
import json
import sys
...
GROUND_TRUTH_PATH = (
    ...
    / "v35_front_seven_index_ground_truth.json"
)
...
def _constants() -> FrontSevenIndexConstants:
    c = GROUND_TRUTH["constants"]
    return FrontSevenIndexConstants(
...
def _history(t: dict) -> FrontSevenTeamHistory:
...
def test_front_seven_index_parity_full_chain(team):
    constants = _constants()
    history = _history(team)
    result = compute_front_seven_index(history, constants)
    ...
    assert result.score == pytest.approx(team["excel_score"], abs=1e-6), (
        f"{team['team']} Team Front 7 Score mismatch: "
