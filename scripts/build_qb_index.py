...
def build(workbook_path: str) -> dict:
    ...
    # ==== claude_code_spec_current_season_blending.md: blend Section 3's Projected
    # Baseline with a real Current-Season value, using the EXACT SAME Blend Weight
    # mechanism Team Ratings' own PPG blend already uses (Model Assumptions C12/C13/C14) --
    # not a new formula. Games Played is cross-referenced from Team Ratings' own col H
    # (already the single real source of truth per team) rather than re-entered here.
    proj_baseline_col = {
    ...
    # ==== Section 4: League average/std-dev of Section 3's BLENDED Baselines ==============
    # Reads from blended_cols (Current-Season-blended), not the static proj_baseline_col,
    # per the spec's own explicit instruction: "The Z-scoring, weighting, and points-score
    # steps downstream of this should reference the newly-blended value."
