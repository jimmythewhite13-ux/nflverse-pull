...
Two stages:
  ...
  2. QB Index / Replacement Value / Manual Override table / RB Value Index / WR-TE Value
     Index / Kicking Index / Offensive Line Index / Availability Index -- fully REBUILT
     from scratch each run (not a Section-1-only value refresh), because their row counts
     are inherently dynamic: which players currently qualify as a Starter/Backup, how many
     games have been played this season, etc. Skipped, non-fatally, on a workbook that
...
def _rebuild_qb_and_availability(workbook_path: str) -> None:
    ...
    # build_kicking_index, and build_oline_index, however, EACH rewrite Team Ratings' Net
    # Power Rating (column N) formula wholesale with whatever set of adjustment terms that
    # script knows about -- build_oline_index's version is the most complete (includes the
    # QB/RB/Kicking/OL terms), so it must run LAST, after build_kicking_index, or an older
