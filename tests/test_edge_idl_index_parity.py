"""
Parity test for prediction_audit/engine/edge_idl_index.py against v35's own real
recalculated EDGE-IDL Index values -- 214 real players, 3 real metrics, no blend step.
"""
import json
import sys
...
GROUND_TRUTH_PATH = (
    ...
    / "v35_edge_idl_index_ground_truth.json"
)
...
def _constants() -> EdgeIdlIndexConstants:
    c = GROUND_TRUTH["constants"]
    return EdgeIdlIndexConstants(
...
def _history(p: dict) -> EdgeIdlPlayerHistory:
...
def test_ground_truth_has_real_players():
    assert len(GROUND_TRUTH["players"]) == 214


@pytest.mark.parametrize(
    "player", GROUND_TRUTH["players"], ids=[p["player_id"] for p in GROUND_TRUTH["players"]],
)
def test_edge_idl_index_parity_full_chain(player):
    constants = _constants()
    history = _history(player)
    result = compute_edge_idl_index(history, constants)
    ...
    assert result.score == pytest.approx(player["excel_score"], abs=1e-6), (
        f"{player['player_name']} EDGE/IDL Index Score mismatch: "
