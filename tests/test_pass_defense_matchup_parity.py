"""
Parity test for prediction_audit/engine/pass_defense_matchup.py against v35's own real
recalculated Pass Defense Matchup values -- 32 real teams, 5 real "Allowed" metrics, all
sign-flipped, no current-season blend step.
"""
import json
import sys
...
GROUND_TRUTH_PATH = (
    ...
    / "v35_pass_defense_matchup_ground_truth.json"
)
...
def _constants() -> PassDefenseMatchupConstants:
    c = GROUND_TRUTH["constants"]
    return PassDefenseMatchupConstants(
...
def _history(t: dict) -> PassDefenseTeamHistory:
...
def test_pass_defense_matchup_parity_full_chain(team):
    constants = _constants()
    history = _history(team)
    result = compute_pass_defense_matchup(history, constants)
    ...
    assert result.score == pytest.approx(team["excel_score"], abs=1e-6), (
        f"{team['team']} Pass Defense Score mismatch: "
