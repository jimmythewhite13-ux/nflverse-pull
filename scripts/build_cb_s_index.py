"""
Builds "CB-S Index" -- Part B of claude_code_spec_defensive_player_index.md. Real,
individual-player, rate-based (per real defensive snap, NOT raw counting totals) coverage
production for the 5 real secondary slots every team's current depth chart identifies: LCB,
RCB, NB, FS, SS -- all FIXED real 1:1 positions (verified live before building this: 32/32
real teams populate LCB/RCB/NB/FS, 31/32 for SS -- unlike EDGE/IDL/LB, no scheme-variable
priority remapping is needed here, so each real position IS its own scored slot directly).

HONESTY REQUIREMENT, stated plainly here and on the tab itself: these are real counting
stats (interceptions, pass breakups), genuinely attributed to individual players in public
data -- but they are noisier and more scheme/teammate-dependent than QB Index's EPA-based
metrics. A cornerback's INT/PBU total is meaningfully influenced by the front seven's real
pass rush, the defensive scheme, and even QB decision-making (a shutdown corner who's simply
never targeted racks up FEWER real INT/PBU opportunities than a more exploitable one), in a
way a QB's EPA/play isn't influenced by his receivers to the same degree. This tab does NOT
carry the same implied confidence as QB Index, and says so on its own closing note.

This does NOT replace Secondary Index (team-level, still Built & Verified, unchanged) -- it's
an additional, complementary layer: individual player scoring feeding a real per-SLOT
Replacement Value, the same relationship QB Index has to Advanced Efficiency Metrics' team-
level passing numbers.

Real data pipeline (defense_stats.py, see that module's own docstring for the full real-data
reasoning): compute_player_season_secondary_stats() (real per-player-per-season INT/PBU,
already given 3 years of pbp) -> compute_player_season_defensive_rates() (joins real per-
player-season snap counts, excludes below-MIN_QUALIFYING_DEFENSIVE_SNAPS seasons entirely,
converts to real per-snap rates, attaches the REAL per-season rookie flag from the start).

NO Section 2B: same real-data finding as EDGE/IDL Index and LB Index -- api.fantasycalc.com's
dynasty-value dataset carries zero defensive-position entries at all, so a zero-history
rookie uses the flat Section 2 Rookie Baseline directly, same as Kicking Index's own
precedent.

Section 6: Replacement Value PER REAL SLOT (LCB vs its own real backup, RCB vs its own, NB
vs its own, FS vs its own, SS vs its own), summed into ONE Team Ratings adjustment column.

Usage:
    uv run python scripts/build_cb_s_index.py "C:\\path\\to\\NFL_Prediction_Model.xlsx"
"""
...
SHEET_NAME = "CB-S Index"
LB_SHEET = "LB Index"
SECONDARY_SHEET = "Secondary Index"
TEAM_RATINGS_SHEET = "Team Ratings"

# All 5 real, FIXED 1:1 secondary positions -- each real position IS its own scored slot
# directly, no scheme-variable priority remapping needed (unlike EDGE/IDL/LB).
SECONDARY_POSITIONS = ["LCB", "RCB", "NB", "FS", "SS"]

METRICS = [
    {"key": "int", "col": "INT Rate", "label": "INT Rate\n(per snap)", "fmt": "0.000"},
    {"key": "pbu", "col": "PBU Rate", "label": "PBU Rate\n(per snap)", "fmt": "0.000"},
]
...
def add_model_assumptions_weights(wb: openpyxl.Workbook) -> None:
    ...
    title = ws.cell(row=title_row, column=1, value=(
        "CB/S Index Weighting & Conversion (pts per std. dev.; reuses QB Index's points-"
        "scale constants C37/C38 -- see 'CB-S Index' tab)"
    ))
    title.font = HEADER_FONT
    title.fill = HEADER_FILL

    rows = [
        (113, "INT Rate Weight (CB/S Index, pts per SD)", 0.5,
         "Real, individually-credited interceptions per real defensive snap -- the "
         "clearest single individual coverage playmaking signal, weighted highest, "
         "same logic as Secondary Index's own team-level weighting."),
        (114, "PBU Rate Weight (CB/S Index, pts per SD)", 0.5,
         "Real pass breakups per real defensive snap -- weighted equal with INT Rate: "
         "a PBU is a real, credited coverage win that simply didn't result in a "
         "turnover, not an inherently lesser signal."),
        (115, "CB/S Index Replacement Value Points-to-Game-Points Conversion", 0.05,
         ...
         "tab's own conversion factor. These stats are noisier and more scheme/"
         "teammate-dependent (see this tab's own closing note), so a given point "
         "gap here should move the prediction less than the same gap in QB/RB "
         "Index."),
    ]
...
def build(workbook_path: str) -> dict:
    ...
    override_roles = [
        role for real_pos in SECONDARY_POSITIONS
        for role in POSITION_ROLE_LABELS[real_pos].values()
    ]
    ...
    print(f"Pulling {HISTORICAL_YEARS} play-by-play data for CB/S rate stats...")
    pbp = fetch_pbp(HISTORICAL_YEARS)
    raw_counts = compute_player_season_secondary_stats(pbp)
    ...
    season_stats = compute_player_season_defensive_rates(
        raw_counts, ["INT", "PBU"], snaps, rosters
    )
    print(f"{len(season_stats)} qualifying CB/S-style player-seasons "
          f"(>= real snap threshold, real rookie flag attached).")

    print(f"Pulling {CURRENT_ROSTER_YEAR} depth charts for CB/S slots...")
    depth_charts = fetch_depth_charts([CURRENT_ROSTER_YEAR])
    current_starters = compute_current_starters(depth_charts)
    slots = pd.concat(
        [resolve_slot_with_backup(current_starters, overrides, pos, pos)
    ...
    if SHEET_NAME in wb.sheetnames:
        del wb[SHEET_NAME]
    for candidate in (LB_SHEET, SECONDARY_SHEET):
    ...
    t = ws.cell(row=1, column=1, value=(
        "CB/S Index -- Real, Individual, Per-Snap-Rate Coverage Production (INT Rate, "
        "PBU Rate) for the 5 real secondary slots (LCB/RCB/NB/FS/SS). COMPLEMENTARY to "
        "Secondary Index (team-level, unchanged) -- adds a real individual-player "
        "Replacement Value layer. NOISIER than QB Index -- see the closing note before "
        "treating these scores with the same confidence."
    ))
    ...
    _section_title(
        ...
        "5 real secondary slots -- both sides of each pair are scored. Missing years "
    ...
    # ==== Section 5: Z-scores, weighted composite, points-scale CB/S Score =================
    ...
    _section_title(
        ws, sec5_title_row, 9,
        "Section 5 — Z-Scores and CB/S Index Score (both metrics are \"higher is "
    ...
    weight_cells = {"int": "$C$113", "pbu": "$C$114"}
    ...
    for i, s in enumerate(slots.to_dict("records")):
        ...
        rv_game = ws.cell(row=row, column=8, value=(
            f'=IF(G{row}="","",G{row}*\'Model Assumptions\'!$C$115)'
        ))
    ...
    note = ws.cell(row=note_row, column=1, value=(
        ...
        "counting stats (interceptions, pass breakups), genuinely attributed to "
        ...
        "QB Index's EPA-based metrics -- a cornerback's INT/PBU total is meaningfully "
        "influenced by the front seven's real pass rush, the defensive scheme, and even "
        "QB decision-making (a shutdown corner rarely targeted racks up FEWER real "
        "opportunities than a more exploitable one). DO NOT read these scores with the "
        "same implied confidence as QB Index. Real per-season rookie identification "
        "(current_roster.attach_real_rookie_season) was used from the START on this tab. "
        "Replacement Value (Section 6) is computed PER REAL SLOT -- LCB/RCB/NB/FS/SS "
        "each have their own real Starter and (where a team has one) real Backup, all "
        "FIXED real 1:1 positions (verified live: no scheme-variable remapping needed "
        "here, unlike EDGE/IDL/LB). NO man/zone coverage tendency, no coverage-shell "
        "classification anywhere on this tab -- confirmed proprietary (paid FTN/Fantasy "
        "Points charting only), not approximated with a fabricated proxy. The 2026 "
        "depth-chart snapshot this tab's population is built from was pulled BEFORE "
        "final 53-man roster cuts -- same caveat as every other current-roster-driven "
    ...
    tr_col = 27  # AA -- next free after LB Index's Z (26)
    tr.cell(row=2, column=tr_col, value="CB/S Repl.\nValue Adj (pts)").font = HEADER_FONT
    ...
    tr_note = tr.cell(row=note_row_tr, column=1, value=(
        "CB/S Repl. Value Adj (AA) sums ALL of that team's real per-slot Replacement "
        "Value (Game Points) from 'CB-S Index' Section 6 (5 real slots: LCB/RCB/NB/FS/"
        "SS) into one Team Ratings column, unconditional. Applies to Net Power Rating "
        "(N) for every team, alongside every other adjustment."
    ))
    ...
    print(
        ...
        f"players, Section 6 {n_slots} real slots. Wired into '{TEAM_RATINGS_SHEET}' (col AA)."
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print('Usage: uv run python scripts/build_cb_s_index.py "path/to/workbook.xlsx"')
        sys.exit(1)
    build(sys.argv[1])
